...
const authenticate = require('../../shared/middleware/authenticate.js');
...
router.post('/', async (req, res) => {
    try {
        Logger.info(`Request received to create a new cart for user ID: ${req.user._id}`);
...
router.post('/products/:productId', async (req, res) => {
    ...
    try {
        Logger.info(`Request received to add product ${productId} to cart for user ID: ${req.user._id}`);
        const cart = await cartService.addProductToCart(req.user._id, productId);
...
router.delete('/products/:productId', async (req, res) => {
    const { productId } = req.params;
    try {
        Logger.info(`Request received to remove product ${productId} from cart for user ID: ${req.user._id}`);
        const cart = await cartService.removeProductFromCart(req.user._id, productId);
        res.status(200).json(cart);
    } catch (error) {
